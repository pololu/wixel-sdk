...
3. The SDCC installer.  The version number is specified in the SDCCVER
   variable in wixel_tools_mui.nsh.  Update that variable if you want
   to bundle a different version of SDCC.
4. The notepad++ installer.  The version number is specified in the
   NPVER variable in wixel_tools_mui.nsh.  Update that variable if you
   want to bundle a different version of SDCC.
5. Place the SDCC and notepad++ installers in the same directory as the
   wixel-sdk directory, one level above the wixel-sdk files.
6. Download and install doxygen (http://www.stack.nl/~dimitri/doxygen/),
   which is used to build the documentation files.
7. With doxygen installed, open a command-line window (start->run->cmd) and
   navigate to your wixel-sdk folder
...
3. Compile build_tools.nsi from the command line, passing it the the location of the loose files as the variable "STARTDIR"
...
4. Compile wixel_tools_mui.nsi from the command line, passing it the location of the wixel-sdk folder as the variable "STARTDIR"
